angular.module('vollyboard').factory('StopwatchFactoryFour', ['$interval','socket','ScheduledMatchesFactory',function($interval,socket,ScheduledMatchesFactory){
    ...
    self.takeTimeout = function(){
        if(self.options[0].matchState!="TOT" && self.options[0].matchState!="BRK")
        {
            ...
            self.updateMatchTime();
            socket.emit('time out started',self.options[0].runningMatchId);
    ...
    self.startTimer = function(){
        if(self.running === false){
            ...
            if(!self.options[0].matchSavedTime){
             self.updateMatchStatusAndDate();
         }

         socket.emit('timer started', self.options[0].runningMatchId);
     }


 };

self.startTimerEvent = function(){
    if(self.running === false){
    var date;
    switch(self.options[0].matchState){
        case "BRK":
...
self.stopTimerEvent = function(){
 ...
 self.stopTimer = function(){
    ...
    socket.emit('timer stopped', self.options[0].runningMatchId);
